...
class Pagination {
        ...
        return new Promise((resolve, reject) => {
            ...
            conn.query([this.query, 'SELECT FOUND_ROWS() AS FOUND_ROWS'].join(';'), this.params, (err, results)=>{
                if (err) {
                    ...
                    this.totalPages = Math.ceil(this.total / this.itensPerPage); //Math.ceil converte arredondando pra cima
                    this.currentPage++;
    ...
    getNavigation(params){
        ...
        //estou no meio da navegação
        else {
            nrstart = this.getCurrentPage() - parseInt(limitPageNav/2);
            nrend = this.getCurrentPage() + parseInt(limitPageNav/2);
        }
        //getCurrentPage() traz a página atual. limitPagesNav/2 para ficar sempre no meio
        ...
        } //faz as setas pra frente e pra tras
